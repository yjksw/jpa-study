...
public class FetchJoinTest {
    ...
    void collectionFetchJoin_DuplicatedDataSelected() {
        ...
        assertThat(teamsWithMembersNoDistinct.size())
            .isEqualTo(6); // members에 대한 fetch join을 할 경우(distinct 없이) 데이터 뻥튀기가 발생한다.
        ...
        assertThat(teamsWithMembersDistinct.size())
            .isEqualTo(2); // members에 대한 fetch join을 할 경우(distinct 있음) 데이터 중복이 일어나지 않는다.
    }
    ...
    void findTeam_MultipleBagFetchException() { // 이 테스트 메소드는 성공하지 않는다. bean creating 할 때 이미 오류가 난다.
        //테스트 상황 - Team 엔티티의 Members, Lockers가 모두 FetchType.EAGER로 설정되어 있다.
    ...
    void findAllJoinFetchLockers() {
        ...
        List<Team> teams = teamRepository.findAllJoinFetchLockers(); //모든 teams를 조회할 때 Lockers는 fetch join으로 즉시로딩한다.
        ...
        List<String> lockerNums = teams.stream() //teams를 조회할 때 즉시로딩 했으므로 추가쿼리가 발생하지 않는다.
            .map(Team::getLockers)
            .flatMap(Collection::stream)
            .map(Locker::getName)
            .collect(toList());
    ...
    @DisplayName("두개의 collection에 대한 fetch join을 살행하면 예외가 발생한다.")
    @Test
    void findAllJoinFetchAll() { // 이 테스트 메소드는 성공하지 않는다. bean creating 할 때 이미 오류가 난다.
        //테스트 상황 - Team 엔티티의 Members, Lockers를 한꺼번에 fetch join하는 JPQL을 실행한다.
    ...
    @Test
    void findWithPagination() { //application.properties에 batch size 옵션 적용한 경우
